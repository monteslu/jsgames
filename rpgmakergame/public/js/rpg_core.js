...
ImageCache.prototype.isReady = function() {
    ...
    for (const key in items) {
        ...
        var a1 = !bitmap.isRequestOnly()
        var b1 = !bitmap.isReady()
        if (a1 && b1) {
            return false;
        }
    }
...
// ImageCache.prototype.isReady = function(){
//     var items = this._items;
//     const ready =  !Object.keys(items).some(function(key){
//         var ro = items[key].bitmap.isRequestOnly()
//         var ir = items[key].bitmap.isReady()

//         var r = !ro && !ir;
//         if(r){
//             console.log("Image cache not ready: ", key)
//         }
//         return r
//     });
...
Bitmap.prototype.decode = function(){
    switch(this._loadingState){
        ...
        case 'requesting': case 'decrypting':
            ...
            if (!this._loader) {
                console.log("Bitmap.prototype.decode this._url", this._url)
...
Graphics.loadFont = function(name, url) {
    // hack
    const font = new FontFace(name, `url(../public/${url})`);
    const loadedFont = font.load().then(resp => {
        console.log("Uploaded font : ", resp)
        document.fonts.add(resp);
    });
...
Graphics._createFontLoader = function(name) {
    console.log("Graphics._createFontLoader empty")
    // hack
...
Window.prototype._refreshCursor = function() {
    ...
    if (w > 0 && h > 0 && this._windowskin) {
        ...
        var p = 96;
        var q = 48;
        console.log(p+m, p+m, q-m*2, q-m*2, ox+m, oy+m, w-m*2, h-m*2)
        ...
        if (retryCount < retryInterval.length) {
            ...
            if (url) {
                if (reloaders.length === 0) {
                    ...
                    console.log("ResourceHandler.createLoader:". url)
                    SceneManager.stop();
                }
